...
async function startServer() {
  ...
  // Lazy initialize Gemini API client to prevent crash on empty core key
  ...
  // Futuristic AI command endpoint
  app.post('/api/ai/command', async (req, res) => {
    ...
    try {
      ...
      if (client) {
        ...
        // High-fidelity fallback AI simulation for local environments without keys
...
- "Show me whitepaper files"`;
          ...
          // Find if there's a file name referred
...
How would you like to configure your Shelby Decentralized storage core today?`;
        ...
        // Return mock reply with high precision
        return res.json({
          status: 'success',
          reply: reply,
          action: action,
  ...
  // Serve static files in production layout
